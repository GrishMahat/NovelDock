  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return ListView(
      ...
      children: [
        ...
        slider('Line Height', settings.lineHeight, 1.0, 3.0, settings.lineHeight.toStringAsFixed(1), (v) => notifier.updateLineHeight(v)),

        const SizedBox(height: 16),
        // ── Layout ──
        section('Layout'),
        slider('H Padding', settings.paddingH, 0, 50, '${settings.paddingH.round()}', (v) => notifier.updatePaddingH(v)),
        slider('V Padding', settings.paddingV, 0, 50, '${settings.paddingV.round()}', (v) => notifier.updatePaddingV(v)),
        slider('Paragraph Gap', settings.paragraphSpacing, 0, 40, '${settings.paragraphSpacing.round()}', (v) => notifier.updateParagraphSpacing(v)),
        ...
        // ── Display ──
        section('Display'),
        switchTile('Bionic Reading', 'Bold first half of each word', settings.bionicReading, (_) => notifier.toggleBionicReading()),
        switchTile('Selectable Text', null, settings.selectableText, (_) => notifier.toggleSelectableText()),
        switchTile('Show Time', null, settings.showTime, (_) => notifier.toggleShowTime()),
        if (!Platform.isLinux && !Platform.isMacOS && !Platform.isWindows) ...[
          switchTile('Show Battery', null, settings.showBattery, (_) => notifier.toggleShowBattery()),
          switchTile('Keep Screen On', null, settings.keepScreenOn, (_) => notifier.toggleKeepScreenOn()),
        ],

        const SizedBox(height: 16),
        // ── Scroll ──
        section('Scroll'),
        radio('Continuous', 'continuous', settings.scrollMode, (v) => notifier.updateScrollMode(v!)),
        radio('Paged', 'paged', settings.scrollMode, (v) => notifier.updateScrollMode(v!)),

        if (!Platform.isLinux && !Platform.isMacOS && !Platform.isWindows) ...[
          const SizedBox(height: 16),
          section('Orientation'),
          radio('Auto', 'auto', settings.orientation, (v) => notifier.updateOrientation(v!)),
          radio('Portrait', 'portrait', settings.orientation, (v) => notifier.updateOrientation(v!)),
          radio('Landscape', 'landscape', settings.orientation, (v) => notifier.updateOrientation(v!)),
        ],
